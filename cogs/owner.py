...
log = logging.getLogger("sentinel.owner")
...
class Owner:
    ...
    def __init__(self, bot):
        ...
        self.disabled_commands = dataIO.load_json("data/sentinel/disabled_commands.json")
        self.global_ignores = dataIO.load_json("data/sentinel/global_ignores.json")
    ...
    async def _set(self, ctx):
        """Changes Sentinel's core settings"""
    ...
    async def owner(self, ctx):
        ...
        if self.bot.settings.no_prompt is True:
            await self.bot.say("Console interaction is disabled. Start Sentinel "
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets Sentinel's global prefixes
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets Sentinel's prefixes for this server
    ...
    async def game(self, ctx, *, game=None):
        """Sets Sentinel's playing status
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets Sentinel's streaming status
    ...
    async def _blacklist_add(self, user: GlobalUser):
        """Adds user to Sentinel's global blacklist"""
    ...
    async def _blacklist_remove(self, user: GlobalUser):
        """Removes user from Sentinel's global blacklist"""
    ...
    async def whitelist(self, ctx):
        ...
        be able to use Sentinel"""
        if ctx.invoked_subcommand is None:
            await self.bot.send_cmd_help(ctx)

    @whitelist.command(name="add")
    async def _whitelist_add(self, user: GlobalUser):
        """Adds user to Sentinel's global whitelist"""
    ...
    async def _whitelist_remove(self, user: GlobalUser):
        """Removes user from Sentinel's global whitelist"""
    ...
    async def restart(self, silently : bool=False):
        """Attempts to restart Sentinel

        Makes Sentinel quit with exit code 26
    ...
    async def contact(self, ctx, *, message : str):
        ...
        if isinstance(author, discord.Member):
            ...
            colour = discord.Colour.sentinel()
    ...
    async def info(self):
        ...
        about = (
            "This is an instance of [Sentinel, an open source Discord bot]({}) "
            "created by [Twentysix]({}) and improved by many.\n\n"
            "Sentinel is backed by a passionate community who contributes and "
            "creates content for everyone to enjoy. [Join us today]({}) "
            "and help us improve!\n\n"
            "".format(red_repo, author_repo, server_url))

        embed = discord.Embed(colour=discord.Colour.sentinel())
        embed.add_field(name="Instance owned by", value=str(owner))
        embed.add_field(name="Python", value=py_version)
        embed.add_field(name="discord.py", value=dpy_version)
        embed.add_field(name="About Sentinel", value=about, inline=False)
    ...
    async def version(self):
        """Shows Sentinel's current version"""
    ...
    def _get_version(self):
        if not os.path.isdir(".git"):
            msg = "This instance of Sentinel hasn't been installed with git."
    ...
    def save_global_ignores(self):
        dataIO.save_json("data/sentinel/global_ignores.json", self.global_ignores)

    def save_disabled_commands(self):
        dataIO.save_json("data/sentinel/disabled_commands.json", self.disabled_commands)
...
def check_files():
    if not os.path.isfile("data/sentinel/disabled_commands.json"):
        print("Creating empty disabled_commands.json...")
        dataIO.save_json("data/sentinel/disabled_commands.json", [])

    if not os.path.isfile("data/sentinel/global_ignores.json"):
        ...
        dataIO.save_json("data/sentinel/global_ignores.json", data)
