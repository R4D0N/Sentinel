...
#
# Sentinel, a Discord bot by Twentysix, based on discord.py and its command
#                             extension.
#
#                   https://github.com/Twentysix26/
#
#
# sentinel.py and cogs/utils/checks.py both contain some modified functions
#                     originally made by Rapptz.
#
#                 https://github.com/Rapptz/RoboDanny/
#

description = "Sentinel - A multifunction Discord bot by Twentysix"
...
class Bot(commands.Bot):
    ...
    async def shutdown(self, *, restart=False):
        """Gracefully quits Sentinel with exit code 0

        If restart is True, the exit code will be 26 instead
        The launcher automatically restarts Sentinel when that happens"""
...
def check_folders():
    folders = ("data", "data/sentinel", "cogs", "cogs/utils")
...
def interactive_setup(settings):
    ...
    if first_run:
        print("Sentinel - First run configuration\n")
    ...
    if first_run:
        ...
        print("\nThe configuration is done. Leave this window always open to"
              " keep Sentinel online.\nAll commands will have to be issued through"
              " Discord's chat, *this window will now be read only*.\n"
              "Please read this guide for a good overview on how Sentinel works:\n"
              "https://twentysix26.github.io/Red-Docs/sentinel_getting_started/\n"
...
def set_logger(bot):
    ...
    sentinel_format = logging.Formatter(
        '%(asctime)s %(levelname)s %(module)s %(funcName)s %(lineno)d: '
        '%(message)s',
        datefmt="[%d/%m/%Y %H:%M]")

    stdout_handler = logging.StreamHandler(sys.stdout)
    stdout_handler.setFormatter(sentinel_format)
    ...
    fhandler.setFormatter(sentinel_format)
    ...
    handler = logging.FileHandler(
        filename='data/sentinel/discord.log', encoding='utf-8', mode='a')
...
def set_cog(cog, value):  # TODO: move this out of sentinel.py
    data = dataIO.load_json("data/sentinel/cogs.json")
    data[cog] = value
    dataIO.save_json("data/sentinel/cogs.json", data)
...
def load_cogs(bot):
    ...
    try:
        registry = dataIO.load_json("data/sentinel/cogs.json")
    except:
        registry = {}
    ...
    if owner_cog is None:
        print("The owner cog is missing. It contains core functions without "
              "which Sentinel cannot function. Reinstall.")
        exit(1)

    if bot.settings._no_cogs:
        bot.logger.debug("Skipping initial cogs loading (--no-cogs)")
        if not os.path.isfile("data/sentinel/cogs.json"):
            dataIO.save_json("data/sentinel/cogs.json", {})
        return
    ...
    dataIO.save_json("data/sentinel/cogs.json", registry)
