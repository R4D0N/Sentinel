...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Sentinel - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Sentinel",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Sentinel in case of issues",
                        action="store_true")
    parser.add_argument("--update-sentinel",
                        help="Updates Sentinel (git)",
                        action="store_true")
...
def update_sentinel():
    try:
        ...
        return
    if code == 0:
        print("\nSentinel has been updated")
    else:
        print("\nSentinel could not update properly. If this is caused by edits "
              "you have made to the code you can try the repair option from "
              "the Maintenance submenu")


def reset_sentinel(reqs=False, data=False, cogs=False, git_reset=False):
    if reqs:
        try:
    ...
    if git_reset:
        ...
        if code == 0:
            print("Sentinel has been restored to the last local commit.")
        else:
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Sentinel:")
        print("1. Update Sentinel + requirements (recommended)")
        print("2. Update Sentinel")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Sentinel (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_sentinel(data=True)
                wait()
        elif choice == "3":
            reset_sentinel(reqs=True)
            wait()
        elif choice == "4":
            print("Are you sure? This will wipe ALL your Sentinel's installation "
                  "data.\nYou'll lose all your settings, cogs and any "
                  "modification you have made.\nThere is no going back.")
            if user_pick_yes_no():
                reset_sentinel(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_sentinel(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Sentinel. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "sentinel.py")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Sentinel...")
                continue
            else:
                if not autorestart:
                    break

    print("Sentinel has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Sentinel without going
    ...
    start_sentinel = "{} --start".format(call)
    start_sentinel_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_sentinel             = ccd + start_sentinel             + pause
    start_sentinel_autorestart = ccd + start_sentinel_autorestart + pause

    files = {
        "start_sentinel"             + ext : start_sentinel,
        "start_sentinel_autorestart" + ext : start_sentinel_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Sentinel Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        if not is_git_installation:
            print("WARNING: It doesn't look like Sentinel has been "
        ...
        print("1. Run Sentinel /w autorestart in case of issues")
        ...
        choice = user_choice()
        if choice == "1":
            run_sentinel(autorestart=True)
        elif choice == "2":
            run_sentinel(autorestart=False)
        elif choice == "3":
            update_menu()
        elif choice == "4":
            requirements_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Sentinel needs Python 3.5 or superior. Install the required "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Sentinel cannot work without the pip module. Please make sure to "
    ...
    if INTERACTIVE_MODE:
        ...
        run_sentinel(autorestart=args.auto_restart)
